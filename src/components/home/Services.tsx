...
const Services = () => {
  return (
    <div>
      <h1 className="text-3xl md:text-4xl lg:text-5xl font-semibold text-black text-center pt-10 px-16 md:px-20 lg:px-24">Smili food un outil tout-en-un pour développer votre activité grâce à Internet</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-14 mt-24 px-14">
        {
          services.map((service) => (
            <div key={service.id} className={`${service.id === 1 && "bg-[#FFF9E9]"} ${service.id === 2 && "bg-[#FFE7C3]"} ${service.id === 3 && "bg-[#FFCA7A]"} text-center pt-20 px-8 pb-5 rounded-lg`}>
              <h2 className={`${service.id === 1 && "text-[#CF8A28]"} ${service.id === 2 && "text-[#CC7A00]"} ${service.id === 3 && "text-[#864F00]"} text-3xl md:text-4xl font-extrabold mb-8`}>{service.title}</h2>
